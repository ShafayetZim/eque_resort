...
class IncomingInvoice(models.Model):
    ...
    def item_count(self):
        return IncomingInvoice_Item.objects.filter(invoice = self).aggregate(Sum('quantity'))['quantity__sum']


# purchase invoice items
class IncomingInvoice_Item(models.Model):
    invoice = models.ForeignKey(IncomingInvoice, on_delete=models.CASCADE)
    product = models.ForeignKey(Product, on_delete=models.CASCADE)
    stock = models.ForeignKey(Stock, on_delete=models.CASCADE, blank=True, null=True)
    buy_price = models.FloatField(default=0)
    price = models.FloatField(default=0)
...
@receiver(models.signals.post_save, sender=IncomingInvoice_Item)
def incoming_stock_update(sender, instance, **kwargs):
    stock = Stock(product=instance.product, quantity=instance.quantity, type=1)
    stock.save()
    # stockID = Stock.objects.last().id
